...
export const TableRow = (props) => {
  const { tableState, updateData, symbols, setSymbols, getLtp } = useContext(TableContext);
  ...
  return (
    <>
        <tr style={{color: getColor(tableState[props.symbol]["orderType"]), backgroundColor: getBackgroundColor(tableState[props.symbol]["exitStatus"])}}>
            <td className="border-b" style={{textAlign: "left", position: "relative", left: "1.2vw"}}>{props.symbol}</td>
            ...
            <td className=" border-b">{tableState[props.symbol]["orderId"]}</td>
